		...
		<div class="main-content">
			<table class="table">
				...
				<c:forEach items="${marksList}" var="marks">
					<tr align="center">
						<td class="center">${marks.realname}</td>
						<td class="center">${marks.srealname}</td>
						<td class="center">${marks.score}</td>
						<td class="center">${marks.addtime}</td>
						<td class="center">${marks.memo}</td>
					</tr>
				</c:forEach>
			</table>
			<ul class="pagination">
				<li>
					<form action="marks/queryMarksByCond.action" name="myform" method="post">
						<label>查询条件: <select name="cond" style="width: 100px">
								<option value="teacherid">按教师查询</option>
								<option value="studentid">按学生查询</option>
								<option value="score">按考核成绩查询</option>
								<option value="addtime">按考核日期查询</option>
